...
import { convertChordProgressionToSmf, type SmfConverter } from "../smf/smf-export.ts";
import { sendMml, type SendMmlClient } from "../daw/send-mml.ts";

const { chordToMmlMock } = vi.hoisted(() => ({
  chordToMmlMock: vi.fn<(input: string) => string | null>(),
}));

vi.mock("../chords/chord-to-mml.ts", () => ({
